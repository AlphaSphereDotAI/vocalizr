...
def test_project_core_metadata_matches_diff_textually():
    text = _read_pyproject_text()
    assert re.search(r'(?m)^\s*name\s*=\s*"vocalizr"\s*$', text), "Expected project name 'vocalizr'"
    assert re.search(r'(?m)^\s*version\s*=\s*"0\.0\.1"\s*$', text), "Expected version '0.0.1'"
    assert re.search(r'(?m)^\s*readme\s*=\s*"README\.md"\s*$', text), "Expected readme 'README.md'"
    assert re.search(r'(?m)^\s*description\s*=\s*".+?"\s*$', text), "Description should be non-empty"
...
def test_project_dependencies_include_expected_entries(pattern: str):
    text = _read_pyproject_text()
    assert re.search(pattern, text), f"Dependency {pattern} not found in [project].dependencies"
...
def test_project_urls_homepage_and_docs_are_commented_or_absent():
    ...
    assert not re.search(r'(?m)^\s*Homepage\s*=', text), "Homepage should be commented out or absent"
    assert not re.search(r'(?m)^\s*Documentation\s*=', text), "Documentation should be commented out or absent"
...
@pytest.mark.parametrize("dev_dep_pattern", [
...
def test_dependency_groups_dev_contains_expected_tools(dev_dep_pattern: str):
    ...
    assert re.search(dev_dep_pattern, text), f"Dev dependency {dev_dep_pattern} not found under [dependency-groups].dev"
...
def test_uv_index_and_sources_configuration_matches_diff():
    ...
    assert re.search(r'(?m)^\s*name\s*=\s*"pytorch-cu124"\s*$', text), "uv.index name should be 'pytorch-cu124'"
    assert re.search(r'(?m)^\s*url\s*=\s*"https://download\.pytorch\.org/whl/cu124"\s*$', text), \
        "uv.index url should be the CUDA 12.4 wheel index"
    assert re.search(r'(?m)^\s*explicit\s*=\s*true\s*$', text), "uv.index explicit must be true"
    assert "[tool.uv.sources]" in text, "Missing [tool.uv.sources] table"
    assert re.search(r'(?m)^\s*torch\s*=\s*\{\s*index\s*=\s*"pytorch-cu124"\s*\}\s*$', text), \
...
def test_parsed_toml_keypaths_and_values_if_supported():
    ...
    for expected in ["gradio[mcp]>=5.38.0", "kokoro>=0.9.4", "soundfile>=0.13.1", "pip>=25.1.1"]:
    ...
    dev = dg.get("dev") or []
    for expected in ["pytest-emoji>=0.2.0", "pytest-md>=0.2.0", "pytest-mergify>=2025.9.10.1", "ruff>=0.13.1", "ty>=0.0.1a20"]:
        assert expected in dev

    tool = data.get("tool") or {}
    uv = tool.get("uv") or {}
    indexes = uv.get("index") or []
    assert any(i.get("name") == "pytorch-cu124"
               and i.get("url") == "https://download.pytorch.org/whl/cu124"
               and i.get("explicit") is True for i in indexes), "Expected pytorch-cu124 index entry"
